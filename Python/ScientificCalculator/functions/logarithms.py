...
- 20 logarithmic identity functions which compute both sides of each identity
  and return (lhs, rhs) as Decimals for inspection or comparison.
...
# Now implement 20 logarithmic identities as functions that compute both
# sides of each identity and return (lhs, rhs) as Decimals. The naming
# follows identity_{n} with a short docstring describing the identity.


def identity_1_log_mul(base, x, y) -> Tuple[Decimal, Decimal]:
...
def identity_2_log_div(base, x, y) -> Tuple[Decimal, Decimal]:
    """Identity: log_b(x / y) = log_b(x) - log_b(y)"""
    lhs = log(base, to_decimal(x) / to_decimal(y))
    rhs = log(base, x) - log(base, y)
    return lhs, rhs
...
def identity_3_log_pow(base, x, r) -> Tuple[Decimal, Decimal]:
...
def identity_4_change_of_base(new_base, x) -> Tuple[Decimal, Decimal]:
...
def identity_5_log_reciprocal(base, x) -> Tuple[Decimal, Decimal]:
...
def identity_6_log_of_base(base) -> Tuple[Decimal, Decimal]:
    """Identity: log_b(b) = 1"""
    lhs = log(base, base)
    rhs = Decimal(1)
    return lhs, rhs
...
def identity_7_log_of_one(base) -> Tuple[Decimal, Decimal]:
    """Identity: log_b(1) = 0"""
...
def identity_8_log_sqrt(base, x) -> Tuple[Decimal, Decimal]:
...
def identity_9_log_base_power(base, k, x) -> Tuple[Decimal, Decimal]:
...
def identity_10_exp_log_inversion(base, x) -> Tuple[Decimal, Decimal]:
...
def identity_11_log_chain(a, b, c) -> Tuple[Decimal, Decimal]:
...
def identity_12_reciprocal_change(base, x) -> Tuple[Decimal, Decimal]:
...
def identity_13_log_power_sum(base, x, y, p, q) -> Tuple[Decimal, Decimal]:
...
def identity_14_log_root(base, x, n) -> Tuple[Decimal, Decimal]:
...
def identity_15_product_of_powers(base, terms: List[Tuple]) -> Tuple[Decimal, Decimal]:
...
def identity_16_log_negative_power(base, x, r) -> Tuple[Decimal, Decimal]:
    """Identity: log_b(x ** -r) = -r * log_b(x)"""
...
def identity_17_sum_to_log(base, x, y) -> Tuple[Decimal, Decimal]:
    """Convert product inside log to sum: log_b(xy) = log_b(x) + log_b(y) (alias)"""
    return identity_1_log_mul(base, x, y)


def identity_18_change_base_via_10(base, x) -> Tuple[Decimal, Decimal]:
...
def identity_19_log_of_power_product(base, x, y, r) -> Tuple[Decimal, Decimal]:
...
def identity_20_log_scaling(base, x, k) -> Tuple[Decimal, Decimal]:
