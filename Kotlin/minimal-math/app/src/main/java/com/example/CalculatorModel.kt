...
data class CalculatorState(
  ...
  val terminalLines: List<TerminalLine> = listOf(
    TerminalLine(
      output = "Python Math Shell v3.12 (Interactive)\nType 'help' for commands or 'def f(x): return x**2' to define functions.",
      isSystem = true
    )
  ),
...
sealed interface CalculatorAction {
  ...
  data class ExecuteTerminalCommand(val command: String) : CalculatorAction
  data object ClearTerminal : CalculatorAction
  data class ExportTerminalResult(val value: String) : CalculatorAction
}
