...
export const ActiveHeistModal: React.FC<ActiveHeistModalProps> = ({
  ...
  const [showTerminalOnMobile, setShowTerminalOnMobile] = useState<boolean>(false);
  ...
  const [memorySequences] = useState<string[]>(['A4', 'B8', 'F0']);
  ...
  return (
    ...
    >
      <div className="w-full max-w-4xl my-auto max-h-[96vh] flex flex-col bg-neutral-950 border border-emerald-500/40 rounded-xl overflow-hidden shadow-[0_0_50px_rgba(16,185,129,0.2)]">
        
        {/* Heist Header Bar */}
        <div className="flex items-center justify-between px-3 sm:px-4 py-2.5 sm:py-3 bg-neutral-900 border-b border-neutral-800 shrink-0">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <div className="p-1.5 sm:p-2 rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 shrink-0">
              <ShieldAlert className="w-4 h-4 sm:w-5 sm:h-5 animate-pulse" />
            </div>
            <div className="min-w-0">
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] sm:text-xs font-bold text-emerald-400 uppercase tracking-widest shrink-0">
                  LIVE INTRUSION
                </span>
                <span className="text-[10px] sm:text-xs px-1.5 py-0.5 rounded bg-neutral-800 text-neutral-300 truncate">
                  {target.codename}
                </span>
              </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            <div className="text-right">
              <div className="text-[9px] sm:text-[10px] text-neutral-400 uppercase tracking-wider">Elapsed</div>
              <div className="text-xs sm:text-sm font-bold text-neutral-200 flex items-center gap-1">
            ...
            <button
              ...
              className="px-2.5 sm:px-3 py-2 rounded-lg border border-rose-500/40 bg-rose-500/10 text-rose-300 hover:bg-rose-500/20 text-xs font-bold transition-all flex items-center gap-1 min-h-[40px]"
            >
              <XCircle className="w-4 h-4" /> <span className="hidden xs:inline">ABORT</span>
            </button>
          </div>
        </div>

        {/* Traceback Heat & Progress Meter */}
        <div className="px-3 sm:px-4 py-2.5 sm:py-3 bg-neutral-950 border-b border-neutral-800 shrink-0">
          <div className="flex items-center justify-between text-xs mb-1.5 gap-2">
            <div className="flex items-center gap-1.5 sm:gap-2 truncate">
              <span className={`font-bold flex items-center gap-1 text-[11px] sm:text-xs truncate ${
                ...
                <Radio className={`w-3.5 h-3.5 shrink-0 ${tracePercent >= 80 ? 'animate-ping' : ''}`} />
                TRACEBACK: {Math.round(tracePercent)}%
              </span>
              {tracePercent >= 80 && (
                <span className="bg-rose-500/20 text-rose-300 px-1.5 py-0.5 rounded text-[9px] sm:text-[10px] font-bold border border-rose-500/40 animate-pulse shrink-0">
                  ⚠ TENSION ALARM!
                </span>
              )}
            </div>
            <span className="text-neutral-400 text-[10px] sm:text-[11px] shrink-0">
              Vault: <strong className="text-emerald-400">{target.vaultSizeGB}GB</strong>
            </span>
          </div>

          {/* Trace Progress Bar */}
          <div className="w-full h-2.5 sm:h-3 rounded-full bg-neutral-900 overflow-hidden border border-neutral-800 relative">
            <div
        ...
        <div className="grid grid-cols-3 border-b border-neutral-800 bg-neutral-900/60 text-[10px] sm:text-xs text-center font-semibold shrink-0">
          <div className={`py-2 px-1 sm:px-3 flex items-center justify-center gap-1 sm:gap-1.5 border-r border-neutral-800 ${
            ...
            {currentStage > 1 ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" /> : <Lock className="w-3.5 h-3.5 shrink-0" />}
            <span className="truncate">1. Bypass</span>
          </div>
          <div className={`py-2 px-1 sm:px-3 flex items-center justify-center gap-1 sm:gap-1.5 border-r border-neutral-800 ${
            ...
            {currentStage > 2 ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" /> : <Cpu className="w-3.5 h-3.5 shrink-0" />}
            <span className="truncate">2. Decrypt</span>
          </div>
          <div className={`py-2 px-1 sm:px-3 flex items-center justify-center gap-1 sm:gap-1.5 ${
            currentStage === 3 ? 'bg-emerald-500/15 text-emerald-300 border-b-2 border-b-emerald-400' : 'text-neutral-600'
          }`}>
            <Radio className="w-3.5 h-3.5 shrink-0" />
            <span className="truncate">3. Deflector</span>
          </div>
        </div>

        {/* Stage Content Interactive Cockpit */}
        <div className="flex-1 p-3 sm:p-4 overflow-y-auto flex flex-col lg:grid lg:grid-cols-12 gap-3 sm:gap-4">
          
          {/* Main Puzzle Interactive Section */}
              ...
              <div className="space-y-3 sm:space-y-4">
                <div className="p-3 rounded-lg bg-neutral-900 border border-neutral-800">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-xs font-bold text-emerald-400 flex items-center gap-1.5">
                      <Lock className="w-4 h-4" /> FIREWALL PORTS (Lvl {target.firewall})
                    </span>
                  ...
                  <p className="text-xs text-neutral-300 leading-relaxed">
                    Tap each port to override firewall packets, or run the Python zero-day bypass script.
                  </p>
                </div>

                {/* Big Touch-Friendly Port Buttons */}
                <div className="grid grid-cols-3 gap-2 sm:gap-2.5">
                  {targetPorts.map((port, idx) => (
                    <button
                      ...
                      className={`p-3 sm:p-4 rounded-xl border flex flex-col items-center justify-center gap-1.5 transition-all text-xs font-bold min-h-[70px] ${
                        lockedPorts[idx]
                          ? 'bg-emerald-500/20 border-emerald-500/50 text-emerald-300 shadow-[0_0_10px_rgba(16,185,129,0.3)]'
                          : 'bg-neutral-900 border-neutral-700 text-neutral-200 hover:border-emerald-400 hover:bg-neutral-800 active:scale-95'
                      }`}
                    >
                      {lockedPorts[idx] ? (
                        <Unlock className="w-5 h-5 sm:w-6 sm:h-6 text-emerald-400" />
                      ) : (
                        <Lock className="w-5 h-5 sm:w-6 sm:h-6 text-neutral-400" />
                      )}
                      <span className="text-xs sm:text-sm">PORT {port}</span>
                      <span className="text-[9px] sm:text-[10px] font-normal text-neutral-400">
                        {lockedPorts[idx] ? 'BYPASSED ✓' : 'TAP TO INJECT'}
                      </span>
                    </button>
                  ))}
                </div>
                ...
                <div className="pt-1">
                  <button
                    ...
                    className="w-full py-3 px-4 rounded-xl border border-emerald-500/40 bg-emerald-500/15 text-emerald-300 hover:bg-emerald-500/25 font-bold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all shadow-[0_0_15px_rgba(16,185,129,0.2)] min-h-[48px] active:scale-98"
                  >
                    <Zap className="w-4 h-4 sm:w-5 sm:h-5 text-amber-400" />
                    <span>Run tools.bypass() Zero-Day Injection</span>
                  </button>
                </div>
              </div>
            )}
            ...
            {currentStage === 2 && (
              <div className="space-y-3 sm:space-y-4">
                <div className="p-3 rounded-lg bg-neutral-900 border border-neutral-800">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-xs font-bold text-emerald-400 flex items-center gap-1.5">
                      <Cpu className="w-4 h-4" /> RSA-4096 CRACK ({target.vaultSizeGB}GB)
                    </span>
                  ...
                  <div className="text-xs text-neutral-300 flex items-center justify-between">
                    <span className="truncate">Hash: <code className="text-emerald-300">{cipherStream}</code></span>
                    <span className="font-bold text-emerald-400">{Math.round(decryptProgress)}%</span>
                  </div>
                </div>

                {/* Progress bar */}
                <div className="w-full h-3.5 sm:h-4 rounded-lg bg-neutral-900 border border-neutral-800 overflow-hidden relative">
                  <div
                ...
                {/* Big Touch-Friendly Memory Sequence Buttons */}
                <div className="grid grid-cols-3 gap-2 sm:gap-2.5">
                  {memorySequences.map((seq, idx) => (
                    <button
                      ...
                      className={`p-3 sm:p-4 rounded-xl border text-xs font-bold flex flex-col items-center justify-center gap-1.5 transition-all min-h-[70px] ${
                        solvedSequences[idx]
                          ? 'bg-emerald-500/20 border-emerald-500/50 text-emerald-300 shadow-[0_0_10px_rgba(16,185,129,0.3)]'
                          : 'bg-neutral-900 border-neutral-700 text-neutral-200 hover:border-cyan-400 hover:bg-neutral-800 active:scale-95'
                      }`}
                    >
                      <Terminal className="w-4 h-4 sm:w-5 sm:h-5 text-cyan-400" />
                      <span className="text-xs sm:text-sm">BLOCK 0x{seq}</span>
                      <span className="text-[9px] sm:text-[10px] font-normal text-neutral-400">
                        {solvedSequences[idx] ? 'SOLVED ✓' : 'DECRYPT'}
                      </span>
                    </button>
                  ))}
                </div>
                ...
                <button
                  ...
                  className="w-full py-3 px-4 rounded-xl border border-cyan-500/40 bg-cyan-500/15 text-cyan-300 hover:bg-cyan-500/25 font-bold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all shadow-[0_0_15px_rgba(6,182,212,0.2)] min-h-[48px] active:scale-98"
                >
                  <Cpu className="w-4 h-4 sm:w-5 sm:h-5 text-cyan-400 animate-spin" />
                  <span>OVERCLOCK CPU THREADS (+12%)</span>
                </button>
              </div>
            )}
              ...
              <div className="space-y-3 sm:space-y-4">
                <div className="p-3 rounded-lg bg-neutral-900 border border-neutral-800">
                  ...
                  <p className="text-xs text-neutral-300">
                    Tap each relay to bounce forensic packets, lower trace proximity, and finalize vault extraction!
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  {proxyHops.map((hop, idx) => (
                    <div
                      key={idx}
                      className={`p-2.5 sm:p-3 rounded-xl border flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 text-xs transition-all ${
                        hop.status === 'bounced'
                    ...
                    >
                      ...
                      <button
                        ...
                        className={`w-full sm:w-auto px-4 py-2.5 rounded-lg text-xs font-bold border transition-all min-h-[44px] flex items-center justify-center gap-1 active:scale-98 ${
                          hop.status === 'bounced'
          ...
          {/* Right Column: Live Terminal Socket Stream (Collapsible on Mobile) */}
          <div className="lg:col-span-5 flex flex-col bg-black rounded-xl border border-neutral-800 p-2.5 sm:p-3 overflow-hidden">
            <div
              className="text-[11px] font-bold text-neutral-400 pb-1.5 border-b border-neutral-800/80 flex items-center justify-between cursor-pointer lg:cursor-default"
              onClick={() => setShowTerminalOnMobile(!showTerminalOnMobile)}
            >
              <span className="flex items-center gap-1 text-emerald-400">
                <Terminal className="w-3.5 h-3.5" /> EXECUTION SOCKET
              </span>
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-neutral-500 hidden sm:inline">TLS 1.3 / PORT 443</span>
                <span className="lg:hidden text-[10px] text-neutral-400 bg-neutral-900 px-2 py-0.5 rounded border border-neutral-800">
                  {showTerminalOnMobile ? 'Hide Log' : 'View Log'}
                </span>
              </div>
            </div>

            <div className={`overflow-y-auto space-y-1 text-[10px] sm:text-[11px] pt-2 text-neutral-300 transition-all ${
              showTerminalOnMobile ? 'h-32' : 'hidden lg:flex lg:flex-col lg:flex-1 lg:min-h-[140px]'
            }`}>
              {logs.map((log, index) => (
            ...
            <div className="pt-2 border-t border-neutral-800/80 text-[10px] text-neutral-400 flex items-center justify-between">
              ...
              <span className="text-neutral-500">Heist Engine</span>
            </div>
          </div>
